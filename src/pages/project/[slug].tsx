...
const ProjectDetail: NextPage = () => {
    ...
    const whitelistRegistrationInfo = useAppSelector(state => state.project.app.whitelistRegistrationInfo);
    const isRegisteringProject = useAppSelector(state => state.project.app.isRegisteringProject);
    const isPurchasing = useAppSelector(state => state.project.app.isPurchasing);
    ...
    useEffect(() => {
        if (user && project) {
            dispatch(projectActions.appFetchWhitelistRegistrationInfo({
                project: project._id,
                user: user._id,
            }));
        } else {
    ...
    useEffect(() => {
        if (user && connection) {
            connection
                ...
                .then((res) => {
                    const usdcBalance = res.value[0].account.data.parsed.info.tokenAmount.uiAmount;
    ...
    const purchase = useCallback(async (usdc: number) => {
        try {
            ...
            dispatch(projectActions.appPurchase({
        ...
        if (user && project) {
            if (isBefore(new Date(), new Date(project.phrases.whitelist.startDate))) {
                return <Tab.Panel className="overflow-x-auto">
                    <h3 className="text-xxl text-solabGray-100 mb-8 text-center">This
                        project is in preparation</h3>
                </Tab.Panel>;
            }
            if (isAfter(new Date(), new Date(project.phrases.whitelist.startDate)) && isBefore(new Date(), new Date(project.phrases.whitelist.endDate))) {
                return <Tab.Panel className="overflow-x-auto">
                    <h3 className="text-xxl text-solabGray-100 mb-8">Your total
                        lottery
                        ticket(s): <span
                            className="text-solabWhite-500">{getTicketsInfo()}</span>
                    </h3>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div className="rounded-lg bg-solabGray-300 p-4">
                            ...
                            {
                                !user.tier ?
                                    <p className="mt-4">You need more LAB to reach
                                        Tier 1. </p> : <><p className="mt-4">Your
                                        current
                                        tier: {user.tier.name}. You will
                                        automatically
                                        get {user.tier.lotteryTickets} tickets</p>
                                        <div
                                            className="w-44 h-44 relative p-1 gradient-background-1 rounded-lg mt-4">
                                            <div
                            ...
                            <p className="mt-8">Start staking SOLAB to gain more
                                tickets.</p>
                            <Link href="/staking">
                        ...
                        <div className="rounded-lg bg-solabGray-300 p-4">
                            ...
                            <p className="mt-4 test-sm">
                                You can collect social ticket by performing
                                various social task:
                            </p>
                            <div className="mt-4">
                                {
                                    project.communityTasks ? project.communityTasks.map(task => (
                                        <div
                                            ...
                                            <div className="flex items-center pr-2">
                                                ...
                                                <span
                                                    className="ml-2 text-solabGray-100 text-xs sm:text-base">
                                                                                                {task.description} :
                                                    {`${whitelistRegistrationInfo && whitelistRegistrationInfo.communityTasksDone && whitelistRegistrationInfo.communityTasksDone.includes(task.uuid) ? task.rewardTickets : 0}/${task.rewardTickets} collected`}
                                                                                            </span>
                                            </div>
                                            {
                                                whitelistRegistrationInfo && whitelistRegistrationInfo.communityTasksDone && whitelistRegistrationInfo.communityTasksDone.includes(task.uuid) ?
                                                    <div
                                                        className="p-2 rounded border border-solabCyan-500">
                                                        <AiOutlineCheckCircle
                                                            className="h-5 w-5 text-solabCyan-500"/>
                                                    </div> : task.userLinkRequired ?
                                                        <button
                                                            className="p-2 rounded border border-solabCyan-500 text-solabCyan-500"
                                                            onClick={() => {
                                                                dispatch(
                                                                    projectActions.openTaskModal(
                                                                        task
                                                                    )
                                                                );
                                                            }}
                                    ...
                                    <div className="flex items-center pr-2">
                                        <div
                                            ...
                                            <HiOutlineSpeakerphone
                                                className="w-6 h-6 text-solabCyan-500"/>
                                        </div>
                                        <span
                                            className="ml-2 text-solabGray-100 text-xs sm:text-base">
                                                                                                Invite your friends
            ...
            if (isAfter(new Date(), new Date(project.phrases.whitelist.endDate)) && isBefore(new Date(), new Date(project.phrases.sale.startDate))) {
                return <Tab.Panel className="overflow-x-auto">
                    <h3 className="text-xxl text-solabGray-100 mb-8 text-center">Whitelist is closed!</h3>
                </Tab.Panel>;
            }
            if (isAfter(new Date(), new Date(project.phrases.sale.startDate)) && isBefore(new Date(), new Date(project.phrases.sale.endDate))) {
                if (!whitelistRegistrationInfo || whitelistRegistrationInfo.isInWhiteList !== inWhitelistApproved) {
                    return <Tab.Panel className="overflow-x-auto">
                        <div className="rounded-lg bg-solabGray-300 p-4 text-center">
                            ...
                            <div className="mt-10">
                                <h4 className="text-xl font-bold">Whitelist is closed. Sorry you were unlucky this
                                    time</h4>
                                <p className="text-solabGray-100">You can continue to try your luck in the next
                                    round</p>
                            </div>
                        </div>
                    </Tab.Panel>;
                    ...
                    <div className="rounded-lg bg-solabGray-300 p-4">
                        ...
                        <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="relative w-60 h-48 m-auto">
                                <Image src="https://solab-media.s3.ap-southeast-1.amazonaws.com/Success.svg"
                                       layout="fill" alt="success"/>
                            </div>
                            <div>
                                <h4 className="text-solabCyan-500 text-xl font-bold">Congratulation! Your address is
                                    whitelisted</h4>
                                <p className="text-solabGray-100">You've won an allocation in
                                    the <span className="text-solabWhite-500 font-bold">{project.name}</span> public
                                    sale</p>
                                <div
                                    ...
                                    <span
                                        className="text-sm text-solabWhite-500 font-bold">1 {project.token.symbol} = {project.idoPrice} USDC</span>
                                </div>
                                <div
                                    className="mt-4 border-b border-solabGray-50 flex justify-between items-center pb-4">
                                    <span className="text-solabGray-100 text-sm">Your total allocation</span>
                                    <span
                                        className="text-sm text-solabWhite-500 font-bold">{userUsdcLimit} USDC</span>
                                </div>
                                <div
                                    className="mt-4 border-b border-solabGray-50 flex justify-between items-center pb-4">
                                    <span className="text-solabGray-100 text-sm">Open time</span>
                                    <span
                                        className="text-sm text-solabWhite-500 font-bold">
                                        {format(
                                            new Date(
                                                project.phrases.sale.startDate
                                            ),
                                ...
                                <div
                                    className="mt-4 border-b border-solabGray-50 flex justify-between items-center pb-4">
                                    <span className="text-solabGray-100 text-sm">Close time</span>
                                    <span
                                        className="text-sm text-solabWhite-500 font-bold">
                                        {format(
                                            new Date(
                                ...
                                <Formik
                                    ...
                                    validationSchema={Yup.object().shape({
                                        usdc: Yup.number()
                                            ...
                                            .max(userUsdcLimit, 'Must be less than or equal your total allocation')
                                            .required('Required'),
                                    })}
                                        ...
                                        return <Form>
                                                ...
                                                <div className="flex font-bold">
                                                    <div className="relative w-6 h-6">
                                                        <Image
                                                            ...
                                                            alt={project.token.symbol} layout="fill"
                                                            className="rounded-full"
                                                        />
                                                    </div>
                                                    <span className="ml-2 uppercase">{project.token.symbol}</span>
                                                </div>
                                                <Field
                                                    name="token"
                                                    type="number"
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        setFieldValue('usdc', parseFloat(token) * project.idoPrice);
                                                    }
                                                    }
                                                />
                                                <ErrorMessage
                                            ...
                                            <div className="mt-6">
                                                <p className="flex text-xs items-center justify-between">
                                                    <span>Rate</span>
                                                    <span>1 {project.token.symbol} = {project.idoPrice}</span>
                                                </p>
                                                <p className="flex text-xs items-center justify-between">
                                                    <span>Your contribution</span>
                                                    <span>{values.usdc}/{userUsdcLimit}</span>
                                                </p>
                                                <p className="flex text-xs items-center justify-between">
                                                    <span>Contributions</span>
                                                    <span>{values.token} {project.token.symbol}</span>
                                                </p>
                                            </div>
                                            {
                                                    ...
                                                    : <div className="mt-6 text-center">
                                                        {
                                                            isSubmitting
                                                                ...
                                                                : <>
                                                                    ...
                                                                    <p className="text-sm text-solabGray-100 mt-2">Minimum
                                                                        contributions
                                                                        amount
                                                                        is 1.00 USDC</p>
                                                                </>
                            ...
                            <div className="bg-solabGray-300 p-4 rounded-lg">
                                ...
                                <div className="w-full text-center mt-3">
                                    ...
                                    <div className="mt-2">
                                        {
                                            user ?
                                                whitelistRegistrationInfo && whitelistRegistrationInfo.isFollowing ?
                                                    <div
                                                    ...
                                                    >
                                                        {isRegisteringProject ? 'Registering...' : 'Register Now'}
                                                    </button> : <div
