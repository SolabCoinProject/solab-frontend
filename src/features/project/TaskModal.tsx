...
const TaskModal: React.FC = () => {
    const isOpen = useAppSelector(
        (state) => state.project.app.isTaskModalOpen
    );
    const user = useAppSelector((state) => state.user.app.user);
    const openTask = useAppSelector((state) => state.project.app.openTask);
    ...
    return (
        <Transition appear show={isOpen} as={Fragment}>
            <Dialog
                ...
                onClose={() => {
                    dispatch(projectActions.closeTaskModal());
                }}
            >
                <div className="min-h-screen px-4 text-center">
                    ...
                    >
                        <div
                            className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl bg-solabGray-300">
                            {openTask && user && project ? (
                                <>
                                    <Dialog.Title
                                        as="h3"
                                    ...
                                    <div className="mt-2 truncate">
                                        ...
                                        {openTask.content ? (
                                            <div
                                                className="text-solabWhite-500 p-2 border border-solabCyan-500 rounded mt-2 overflow-auto">
                                                {ReactHtmlParser(
                                                    openTask.content
                                                )}
                                            </div>
                                            ...
                                            onSubmit={(values) => {
                                                dispatch(
                                                    projectActions.appDoCommunityTask({
                                                        project: project._id,
                                                        data: {
                                                            taskUuid: openTask.uuid,
                                                            userId: user._id,
                                                            userUrl: values.userPostUrl
                                                        }
                                                    })
                                                );
                                            }}
